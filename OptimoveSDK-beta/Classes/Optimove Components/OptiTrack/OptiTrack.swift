...
extension OptiTrack {
	func report(event: OptimoveEventDecorator, withConfigs config: OptimoveEventConfig) {
		if event is OptimoveCustomEventDecorator {
			guard isEnable else {
				OptiLogger.logOptiTrackDisable()
				return
			}
		}
...
//            let updatedScreenPath = "\(Bundle.main.bundleIdentifier!)/\(screenPath)".lowercased()
			OptiLogger.logReportScreenEvent(screenTitle: screenTitle)
			DispatchQueue.main.async {
				...
				guard let config = Optimove.shared.eventWarehouse?.getConfig(ofEvent: event) else {
					OptiLogger.logConfugurationForEventMissing(eventName: event.name)
					return
				}
	...
	func setUserId(_ userId: String) {
		OptiLogger.logOptitrackSetUserID(userId: userId)
		self.tracker.userId = userId
	}

	func dispatchNow() {
		if RunningFlagsIndication.isComponentRunning(.optiTrack) {
			OptiLogger.logOptitrackDispatchRequest()
			tracker.dispatch()
		} else {
			OptiLogger.logOptitrackNotRunning()
		}
	}
	...
	private func reportUserAgent() {
		...
		if let config = Optimove.shared.eventWarehouse?.getConfig(ofEvent: event) {
			OptiLogger.logOptitrackReport(event: "User Agent")
            let dec = OptimoveEventDecorator(event: event, config: config)
			report(event: dec, withConfigs: config)
		} else {
			OptiLogger.logLoadingConfigsError(ofEvent: "User Agent")
		}
	}
    ...
    private func reportSdkVersion()
    {
        ...
        if let config = Optimove.shared.eventWarehouse?.getConfig(ofEvent: event) {
            OptiLogger.logOptitrackReport(event: "sdk metadata")
            let dec = OptimoveEventDecorator(event: event, config: config)
            report(event: dec, withConfigs: config)
        } else {
            OptiLogger.logLoadingConfigsError(ofEvent: "sdk metadata")
        }
    }
		...
		deviceStateMonitor.getStatus(of: .userNotification) { (granted) in
			if self.isOptInOutStateChanged(with: granted) {
				if granted {
					...
					guard let config = Optimove.shared.eventWarehouse?.getConfig(ofEvent: event) else {
						OptiLogger.logLoadingConfigsError(ofEvent: "opt in")
						return
					}
                    OptiLogger.logOptitrackReport(event: "opt in")
					let dec = OptimoveEventDecorator(event: event, config: config)
					self.report(event: dec, withConfigs: config)
					...
					guard let config = Optimove.shared.eventWarehouse?.getConfig(ofEvent: event) else {
						OptiLogger.logLoadingConfigsError(ofEvent: "opt out")
						return
					}
					OptiLogger.logOptitrackReport(event: "opt out")
					let dec = OptimoveEventDecorator(event: event, config: config)
					self.report(event: dec, withConfigs: config)
...
extension OptiTrack {
	...
	private func reportAppOpen() {
		...
		guard let config = Optimove.shared.eventWarehouse?.getConfig(ofEvent: event) else {
			OptiLogger.logLoadingConfigsError(ofEvent: "app open")
			return
		}
		OptiLogger.logOptitrackReport(event: "app open")
		let dec = OptimoveEventDecorator(event: event, config: config)
		report(event: dec, withConfigs: config)
