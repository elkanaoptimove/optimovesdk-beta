...
class Registrar {
    ...
    init(optipushMetaData: OptipushMetaData) {
        OptiLogger.logRegistrarInitializtionStart()
        ...
        OptiLogger.logRegistrarInitializtionFinish()
    }
    ...
    private func backupRequest(_ mbaasRequestBody: MbaasRequestBody) {
        ...
        if let json = mbaasRequestBody.toMbaasJsonBody() {
            ...
            OptiLogger.logMbaasRequestEncodeError(mbaasRequestBody: mbaasRequestBody.description)
        }
    }
    ...
    private func retryFailedOperation(_ operation: MbaasOperations, using json: Data) {
        ...
        guard path != nil else {
            OptiLogger.logMbaasRequestUrlEncodeError()
            return
        }
        OptiLogger.logSendRetryTo(path: path!.path)
        NetworkManager.post(toUrl: path!, json: json) { (_, error) in
            guard error == nil else {
                OptiLogger.logRetryFailed()
                return
            }
...
extension Registrar: RegistrationProtocol {
    func register() {
        ...
        guard let json = mbaasRequest.toMbaasJsonBody() else {
            OptiLogger.logJsonBuildFailure(mbaasRequestOperation: mbaasRequest.operation.rawValue)
            return
        }
        OptiLogger.logSendMbaasRequest(url: url, json: String(decoding: json, as: UTF8.self))
    ...
    func handleResponseFromMbaas(ofRequest mbaasRequest: MbaasRequestBody, withData data: Data?, error: OptimoveError?, url: URL, didComplete: ((Bool) -> Void)? = nil) {
        ...
        if let error = error, (error == .badRequest || error == .notFound || error == .gone) {
            OptiLogger.logMbaasRequestError(mbaasRequestOperation: mbaasRequest.operation.rawValue, errorDescription: String(decoding: data!, as: UTF8.self))
            didComplete?(false)
            return
        }
        guard error == nil else {
            OptiLogger.logMbaasRequestError(mbaasRequestOperation: mbaasRequest.operation.rawValue, errorDescription: error.debugDescription)
            self.handleFailedMbaasRequest(of: mbaasRequest, to: url)
            didComplete?(false)
            return
        }
        OptiLogger.logMbaasResponse(mbaasRequestOperation: mbaasRequest.operation.rawValue, response: String(decoding: data!, as: UTF8.self))
